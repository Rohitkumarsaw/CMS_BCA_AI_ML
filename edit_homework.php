...
$hw_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
$stmt = $pdo->prepare("SELECT * FROM homework WHERE id = ? AND user_id = ?");
$stmt->execute([$hw_id, $user_id]);
$homework = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$homework) {
    setFlashMessage('danger', 'Homework not found.');
    redirect('homework.php');
}

$errors = [];
$old_file = $homework['file_path'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        $new_file = uploadFile($_FILES['file'], 'homework', ALLOWED_HOMEWORK_TYPES);
        if ($new_file === false) {
    ...
    <div class="app-content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="mb-1"><i class="fas fa-edit me-2 text-primary"></i>Edit Homework</h4>
                    <p class="text-muted mb-0">Update homework assignment</p>
                </div>
...
<form method="POST" action="" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" id="subject" name="subject" list="subject_list" required value="<?= htmlspecialchars($homework['subject'] ?? '') ?>" placeholder="Type or select a subject">
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="description" name="description" rows="4" 
                                              required><?= htmlspecialchars($homework['description']) ?></textarea>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="date" class="form-control" id="due_date" name="due_date" 
                                                   value="<?= $homework['due_date'] ?>" required>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" id="status" name="status" list="status_list" required value="<?= htmlspecialchars($homework['status'] ?? '') ?>" placeholder="Not Submitted, Submitted, Pending, or custom">
                                        </div>
                                    </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="form-text">Allowed: <?= implode(', ', ALLOWED_HOMEWORK_TYPES) ?> | Max size: 5MB. Leave empty to keep current file.</div>
                                </div>
